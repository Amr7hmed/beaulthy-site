...
export default function DeleteBtn({ info, user, update, setPages }) {
  const { confirm } = Modal;

  const success = () => {
    message.success("You Deleted the Vacancy");
  };
  ...
  const options = {
    method: "delete",
    url: `${process.env.REACT_APP_API_BASEURL}/api/admin/vacancies/${info.id}`,
    headers: {
