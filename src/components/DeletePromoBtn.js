...
export default function DeletePromoBtn({ info, user, update, setPages }) {
  const { confirm } = Modal;

  const success = () => {
    message.success("You Deleted Promo");
  };
  ...
  const options = {
    method: "delete",
    url: `${process.env.REACT_APP_API_BASEURL}/api/admin/promos/${info.id}`,
    headers: {
