...
export default function DeleteBlogBtn({ info, user, update }) {
  const { confirm } = Modal;

  const success = () => {
    message.success("You Deleted the blog");
  };
  ...
  const options = {
    method: "delete",
    url: `${process.env.REACT_APP_API_BASEURL}/api/admin/blogs/${info.id}`,
    headers: {
